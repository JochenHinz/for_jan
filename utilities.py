import numpy as np
import scipy as sp
import scipy.interpolate, scipy.sparse, inspect, collections, itertools, copy, functools, abc, pickle, Solver, elasticity
from nutils import *
...
class tensor_grid_object(base_grid_object):
    ...
    def empty_copy(self):  ## create a blank copy of the grid object
        if len(self.ndims) == 2:
            return tensor_grid_object(ischeme = self.ischeme, knots = self._knots, side = self._side, target_space = self._target_space)
        elif len(self.ndims) == 1:
            return tensor_grid_object(self.domain, self.geom, ischeme = self.ischeme, knots = self._knots, side = self._side, target_space = self._target_space)
        else:
            raise NotImplementedError
    ...
    ## verts = [t_0, t_1, t_2, ...], grid_objects = [go(t_0), go(t_1), go(t_2), ...]
        ...
        go = tensor_grid_object.grid_sum(*grid_objects)
    ...
    def detect_defects_discrete(self, ischeme = 10, ref = 0, thresh_ratio = 0):
        ''' 
        ...
        funcevals = self.domain.refine(ref).elem_eval( self.determinant, ischeme=gauss(ischeme)) > 0
    ...
    def grid_unification(*args):
        go = tensor_grid_object.grid_sum(*args)
    ...
    def fromxml(path, index = 0):
        ...
        go = make_go('bspline', knots = np.prod([nonuniform_kv(degrees[j], knotvalues = kvs[j], \
                             knotmultiplicities = kms[j]) for j in range(n_kvs)]), target_space = repeat)
        s = np.array(xml[1].text.split(), dtype = float)
        l = len(s) // repeat
        d = np.arange(0, len(s),repeat)
        go.s = np.concatenate([s[d_].reshape(list(reversed(go.ndims))).T.flatten() for d_ in [d + k for k in range(repeat)]])
        go.set_cons_from_s()
        return go
...
class multipatch_object(multigrid_object):  ## inherits from multigrid_object, add connectivity
    ...
    def __init__(self, go_list, *args, **kwargs):
    ...
    def fromxml(path):
        xml = ET.parse(path).getroot()
        mgo = []
        for i in range(len(xml) - 1):  ## last entry of .xml is connectivity and can be disregarded
            mgo.append(tensor_grid_object.fromxml(path, index = i))
        return multipatch_object(mgo)
